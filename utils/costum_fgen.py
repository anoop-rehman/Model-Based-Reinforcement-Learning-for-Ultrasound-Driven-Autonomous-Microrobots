...
class FuncGen:
    ...
    def _initialise_model_properties(self):
        ...
        elif np.any(["AFG3011" in a for a in [self._model, self._override_compat]]):
            self.instrument_limits = {
                "frequency lims": ({"min": 1e-6, "max": 5e6}, "Hz"),
                "voltage lims": (
                    {"50ohm": {"min": -10, "max": 10}, "highZ": {"min": -20, "max": 20}},
                    "V",
                ),
                "amplitude lims": (
                    {
                        "50ohm": {"min": 0.00, "max": 10},
                        "highZ": {"min": 0.00, "max": 20},
                    },
                    "Vpp",
                ),
...
class FuncGenChannel:
    ...
    def set_run_mode(self, mode: str):
        ...
        """
        assert mode in {"CW","FIXed","SWEep"}, "Invalid mode"
        self._fgen.write(f"{self._source}FREQuency:MODE {mode}")
        print(f"Set run mode to {mode}")
        
    
    def set_sweep_frequency(self, start_freq: float, stop_freq: float, time: float = 10, unit: str = "Hz"):
        """Set the frequency in Hertz (or mHz, kHz, MHz, see options)
        ...
        NotSetError
            ...
            get function
        """
        assert start_freq < stop_freq, "Start frequency must be less than stop frequency"
        # Check if the given frequency is within the current limits
        min_freq, max_freq = self.get_frequency_lims()
        def check_and_convert(freq, unit):
            freq = _SI_prefix_to_factor(unit) * freq
            if freq < min_freq or freq > max_freq:
                msg = (
                    f"Could not set the frequency {freq}Hz as it is not "
                ...
                raise NotSetError(msg)
            return freq
        for freq, start_stop in zip([start_freq, stop_freq], ["STARt", "STOP"]):
            freq = check_and_convert(freq, unit)
            self._fgen.write(
                f"{self._source}FREQuency:{start_stop} {freq}{unit}",
                custom_err_message=f"set frequency {freq}{unit}",
            )
        print(f"Set sweep frequency from {start_freq} to {stop_freq} in {time} milliseconds")
