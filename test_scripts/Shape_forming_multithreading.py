...
def process_frames(frame_queue, tracker, bottleneck_center, ser, fgen, save_dir, segmented, closeness_factor, passing_factor):
    image_counter = 0
    ...
    while True:
        ...
        # Ensure the frame and segmented image have the same dimensions
        if frame.shape[:2] != segmented.shape[:2]:
            segmented = cv2.resize(segmented, (frame.shape[1], frame.shape[0]))
...
def main(): 
    ...
    # FSM States
    global NORMAL, APPROACH_BOTTLENECK, PASS_BOTTLENECK, CROSS_OBSTACLE
    NORMAL = 0
    ...
    process_thread = threading.Thread(target=process_frames, args=(frame_queue, tracker, bottleneck_center, ser, fgen, save_dir, segmented, closeness_factor, passing_factor))
